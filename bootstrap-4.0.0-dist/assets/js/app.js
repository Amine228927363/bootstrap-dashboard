...
//search for each column 
    $(document).ready(function () {
        // Add event listener to each input field in the header
        $("#myTable thead input").on("keyup", function () {
            var columnIndex = $(this).closest('th').index(); // Get the index of the current column
            var value = $(this).val().toLowerCase();
            
            console.log("Filtering column index:", columnIndex, "with value:", value);
    
            $("#myTable tbody tr").filter(function () {
                var cellValue = $(this)
                    .find("td")
                    .eq(columnIndex) // Get the cell in the corresponding column
                    .text()
                    .toLowerCase();
    
                console.log("Checking cell value:", cellValue);
    
                // Show or hide the row based on whether the cell contains the search value
                $(this).toggle(cellValue.indexOf(value) > -1);
            });
        });
    });
    //search generale 
      ...
      $(document).ready(function () {
        $("#floatingInput").on("input", function () {
          ...
          if (emailPattern.test(email)) {
            ...
            emailFeedback.text("Invalid email").css("color", "red");
